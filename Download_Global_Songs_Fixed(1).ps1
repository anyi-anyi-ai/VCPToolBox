...
foreach ($s in $songs) {
    Write-Host ''
    Write-Host ('>>> Searching: ' + $s) -ForegroundColor Yellow
    ...
    if ($apiRes -and $apiRes.code -eq 200 -and $apiRes.data.url) {
        ...
        if (Test-Path $ffmpeg) {
            ...
            $q = [char]34
            $args = '-y -i ' + $q + $tempPath + $q + ' -b:a 320k ' + $q + $finalPath + $q
        ...
        if ($lrcRes -and $lrcRes.lrc.lyric) {
            ...
            if ($lrcRes.tlyric.lyric -and $lrcRes.tlyric.lyric.Length -gt 10) { $lyricText += [Environment]::NewLine + $lrcRes.tlyric.lyric }
...
if ($added -gt 0) {
    ...
    Write-Host ''
    Write-Host 'No new songs were added.' -ForegroundColor Yellow
}
Write-Host ''
