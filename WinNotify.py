...
# Python 3.12+ pkg_resources 兼容性补丁 (用于 win10toast)
import sys
...
import threading
import time
from win10toast import ToastNotifier # 导入 win10toast 库

# --- 配置信息 (请根据您的 VCP 工具箱设置进行修改) ---
VCP_KEY = '164522' # 请与您 NAS 上 VCP 工具箱的 VCP_Key 保持一致
WS_SERVER_URL = 'ws://192.168.2.179:5890' # 您 NAS 上 VCP 工具箱的 WebSocket 地址
WS_URL = f"{WS_SERVER_URL}/VCPlog/VCP_KEY={VCP_KEY}" # 完整的 WebSocket 连接 URL
APP_ID = "Ryan.VCP.Toolbox.Notifier" # Windows 通知的应用程序 ID，建议设置为您的应用名，以便在通知中心识别

# 初始化 ToastNotifier 对象，用于发送通知
...
def show_notification(title, message):
    ...
    try:
        ...
        notifier.show_toast(
            title,
            message,
            icon_path=None, # 您可以在这里指定一个.ico 文件路径作为通知图标，例如 'D:\\path\\to\\your\\icon.ico'
            duration=5,     # 通知显示时长（秒）
...
def on_message(ws_app, message):
    ...
    try:
        ...
        if data.get('type') == 'vcp_log' and data.get('data'):
            ...
            # 如果 log_data 是一个字符串，尝试将其作为 JSON 解析
            ...
            notification_title = "VCP 工具箱通知" # 默认通知标题
            notification_content = ""             # 默认通知内容

            # 提取通知内容
            if isinstance(log_data, dict):
                # 优先处理 AI Agent 消息 (e.g., {"type": "agent_message", "message": "..."})
...
def on_open(ws_app):
    ...
    # 连接成功时，可以发一个通知提醒主人
    show_notification("VCP 工具箱通知", "Windows 通知监听已连接，准备接收消息！")
