...
function updateTopicIndex(db, id, configPath, config, entityType) {
  ...
  if (topicObj) {
    ...
    upsertEntityIndex(id, entityType, configPath, hash);
  }
}
...
async function deleteEntity({ id, type, deletedAt, appDataPath }) {
  ...
  const safeId = sanitizeId(id);
  const phase = (type === "agent_topic" || type === "group_topic" || type === "topic" || type === "avatar") ? "owner_metadata" : "owner_metadata";
  // Wait, actually avatar is owner_metadata.
  // Topic is topic_metadata.
  const actualPhase = (type === "agent_topic" || type === "group_topic" || type === "topic") ? "topic_metadata" : "owner_metadata";
 ...
 * @returns {Promise<{success: boolean, error?: string}>}
 */
async function deleteMessage({ msgId, deletedAt }) {
  const db = getDb();
  const logger = getLogger();
  if (!db) return { success: false, error: "Database not initialized" };
  ...
  try {
    softDeleteMessageIndex(safeMsgId, deletedAt);
    logger.logOperation("messages", "delete", safeMsgId, "success", "soft deleted");
    return { success: true };
  } catch (e) {
