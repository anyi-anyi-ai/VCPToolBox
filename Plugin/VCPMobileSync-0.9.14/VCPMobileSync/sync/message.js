        ...
        if (msg.attachments && Array.isArray(msg.attachments)) {
          msg.attachments = msg.attachments.map((att) => {
            ...
            return {
              ...
              imageFrames: data.imageFrames || null,
...
async function uploadMessagesBatchRaw(req, appDataPath, res) {
  ...
  // 释放所有 writeIntentLock（文件监控器此时可安全摄入）
  for (const tid of processedTopicIds) {
...
async function ingestHistoryToDb(filePath, topicId, source = "watcher") {
  ...
  try {
    ...
    db.prepare(
      "UPDATE entity_index SET aggregated_hash = ?, updated_at = ? WHERE id = ? AND type = 'topic'",
