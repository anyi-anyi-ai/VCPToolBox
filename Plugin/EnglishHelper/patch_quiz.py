...
pattern = r'def do_quiz_generate\(text\):.*?return \{.*?"explanation":.*?\}'

replace_str = '''def do_quiz_generate(text):
    nlp = get_nlp()
    if not nlp:
    ...
    distractors = []
    cfg = get_config()
    db_path = cfg.get("dictionary", {}).get("providers", {}).get("local_ecdict", {}).get("path", "ecdict_mini.db")
...
print("Quiz generator patched successfully!")